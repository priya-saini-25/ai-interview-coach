...
export const DsaTrackerPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      {/* Header Banner */}
      <div className="glass-panel p-6 rounded-2xl border border-gray-800 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-3">
            <div className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-gray-900/80 border border-gray-800 text-xs font-medium text-gray-200">
              <Briefcase className="w-3.5 h-3.5 text-indigo-400" />
              <span>Target Role: <strong className="text-white">{targetRole || 'Not Set'}</strong></span>
            </div>

            {targetCompany ? (
              <div className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-gray-900/80 border border-gray-800 text-xs font-medium text-gray-200">
                <Building className="w-3.5 h-3.5 text-amber-400" />
                <span>Target Company: <strong className="text-white">{targetCompany}</strong></span>
              </div>
            ) : (
      ...
      <Card className="p-0 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-800 bg-gray-900/60 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">Ordered by Role & Company Relevance</span>
        </div>
          ...
          <div className="divide-y divide-gray-800/60">
            {problems.map((problem) => {
              ...
              return (
                <div
                  key={problem.id}
                  className="p-5 hover:bg-gray-800/40 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                >
                  <div className="space-y-1.5 max-w-2xl">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-base font-bold text-white hover:text-indigo-400 transition cursor-pointer" onClick={() => handleOpenProblem(problem)}>
                        {problem.title}
                      </span>
                      <Badge variant={problem.difficulty === 'Easy' ? 'success' : problem.difficulty === 'Medium' ? 'warning' : 'danger'}>
                        {problem.difficulty}
                      </Badge>
                      <span className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-300 font-medium">
                        {problem.topic}
                      </span>
                      {isSaved && (
                        <span className="inline-flex items-center text-[11px] font-semibold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                          <Bookmark className="w-3 h-3 mr-1 fill-amber-400" /> Saved for Revision
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-1">
                      {problem.companies && problem.companies.slice(0, 3).map((comp) => (
                        <span key={comp} className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-950/60 border border-indigo-500/20 text-indigo-300 font-medium">
                          {comp}
                        </span>
                      ))}
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0 sm:self-center">
                    <Button
                      ...
                      leftIcon={<Bookmark className={`w-3.5 h-3.5 ${isSaved ? 'fill-amber-400 text-amber-400' : ''}`} />}
                      ...
                      className={isSaved ? 'bg-amber-500/10 border-amber-500/30 text-amber-300 hover:bg-amber-500/20' : ''}
                    >
                      {isSaved ? '✓ Saved for Revision' : 'Save for Revision'}
                    </Button>

                    {isSolved ? (
                      <span className="inline-flex items-center text-xs text-emerald-400 font-semibold bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/20">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="glass-panel p-6 rounded-2xl border border-gray-800 max-w-2xl w-full shadow-2xl space-y-6 my-8">
              ...
              <div>
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  {activeProblem.savedForRevision && (
                    <span className="text-xs text-amber-400 font-semibold inline-flex items-center bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                      <Bookmark className="w-3 h-3 mr-1 fill-amber-400" /> Saved for Revision
                    </span>
                  )}
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400">Problem Description</h3>
              <p className="text-sm text-gray-200 leading-relaxed font-sans bg-gray-900/60 p-4 rounded-xl border border-gray-800">
            ...
            {activeProblem.companies && activeProblem.companies.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400">Frequently Asked At</h3>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {activeProblem.examples.map((ex, idx) => (
                    <div key={idx} className="bg-gray-950 p-3 rounded-lg border border-gray-800 font-mono text-xs space-y-1">
                      <p className="text-indigo-300"><strong>Input:</strong> {ex.input}</p>
                      <p className="text-emerald-300"><strong>Output:</strong> {ex.output}</p>
                      {ex.explanation && <p className="text-gray-400 font-sans"><strong>Explanation:</strong> {ex.explanation}</p>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 pt-4 border-t border-gray-800">
              ...
              <Button
                ...
                leftIcon={<Bookmark className={`w-3.5 h-3.5 ${activeProblem.savedForRevision ? 'fill-amber-400 text-amber-400' : ''}`} />}
                onClick={() =>
                ...
                className={activeProblem.savedForRevision ? 'bg-amber-500/10 border-amber-500/30 text-amber-300 hover:bg-amber-500/20' : ''}
              >
              ...
              {activeProblem.status !== 'Solved' ? (
                ...
                <span className="inline-flex items-center text-xs text-emerald-400 font-semibold bg-emerald-500/10 px-3 py-2 rounded-lg border border-emerald-500/20">
                  <CheckCircle2 className="w-4 h-4 mr-1.5" /> Solved
                </span>
              )}
            </div>
